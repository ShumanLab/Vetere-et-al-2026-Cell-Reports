...
function plot_CSD_bygroup_V2_by_speed(animals, filter, region, speedbinsize, binsstart)

%calculate and plot CSD for each animal
%as well as group summary data 
%ADDED LINE TO SAVE FILE FOR EACH ANIMAL 
...
for anim = 1: length(animals)
    ...
    for b = 1:length(speed_bins)-1
        ...
        [lfp, events, group, sex, age, channels_by_layer_csd, remove_chans_csd] = pre_CSD_by_speed(animal, filter, region, mins, maxs);
        ...
        [csd, lfpAvg] = bz_eventCSD_lv_by_speed(lfp, events, animal, group, filter, csd_lyr_borders, region);
...
%        xlabel('CSD Magnitude');ylabel('Channel');
       ...
       %plot csd mag (absolute value across one oscillation cycle)
%        figure('Name', ([animal ' ' group ' CSD magnitude (abs value - 1 cycle)']));
%        if strcmp(group, '6wt')
...
%        end  
%        title([animal ' ' group ' CSD magnitude (abs value - 1 cycle)'])
%        xlabel('CSD Magnitude');ylabel('Channel');  
       ...
       %row per layer per animal
        ...
        %repeat for max abs value for each layer 
...
%        figure('Name', ([animal ' ' group ' Avg CSD magnitude by layer (abs value - 1 cycle)']));
%        if strcmp(group, '6wt')
...
%        end  
%        title([animal ' ' group 'Avg CSD magnitude by layer (abs value - 1 cycle)'])
%        xlabel('CSD Magnitude');ylabel('Channel');  
...
%     end
%     
%      %add each animal's csd mag (avg of 5 time points (ms) around time zero) to a matrix for each group - single value per
%     %channel per animal
%     if group == '6wt'
...
%     end
%     
%     %add each animal's CSD mag (avg of abs val of ~an oscillation cycle's worth of time points (ms) around time zero) to a matrix 
...
%     
    ...
    csd_struct.age = age;
    csd_struct.sex = sex;
    csd_struct.binmin = mins;
    csd_struct.binmax = maxs;
    csd_struct.binsize = speedbinsize;
    csd_struct.numtroughs = length(events); 


    save([exp_dir '\' animal '_' filter '_' region '_CSDrun_' num2str(mins) '_' num2str(maxs) '.mat'],'csd_struct') ;  

    clear  csd_struct 

    disp(['done with speeds ' num2str(mins) ' to ' num2str(maxs)]);
    end 
end
% 
% 
...
%plot only means - not individual animals
